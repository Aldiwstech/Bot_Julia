...
def generate_site_card(site_id):
    ...
    # ---------------------------------------------------------
    # These coordinates are tied to the ':' already printed in mokup2.png.
    # They never depend on label length.
    # SITE INFO is calibrated directly against the colon pixels in mokup2.png.
    # IMPORTANT: every value starts on exactly the same vertical X line.
    # Do not calculate X from label width.
    SITE_VALUE_X = 301
    SITE_VALUE_RIGHT = 466
    SITE_VALUE_W = SITE_VALUE_RIGHT - SITE_VALUE_X
    ...
    site_y = [230, 286, 342, 398, 454, 510, 566, 634, 694, 760]

    for i, (text, y) in enumerate(zip(site_rows, site_y)):
        # Absolute placement: the left edge is ALWAYS the same X.
        # Only font size/width changes to keep long values inside the panel.
        if i == 9:
            start_size, min_size = 16, 13
        elif i in (1, 3, 4, 5, 6):
            start_size, min_size = 19, 15
        else:
            start_size, min_size = 20, 16

        draw_text_fit(
            draw, text, *xy(SITE_VALUE_X, y),
            round(SITE_VALUE_W * sx),
    ...
    rect_y = [216, 260, 304, 349, 394, 439, 484]
    ...
    batt_y = [603, 646, 688, 731, 773, 815]
    ...
    health_y = [216, 260, 306, 352, 399, 445, 492, 539]
